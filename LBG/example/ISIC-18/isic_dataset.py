...
df1 = pd.read_csv('ISIC2018_Task3_Training_GroundTruth.csv')
df2 = pd.read_csv('ISIC2018_Task3_Test_GroundTruth.csv')
df3 = pd.read_csv('ISIC2018_Task3_Validation_GroundTruth.csv')


def get_cat(img, df=df1):
...
def gray_world(img):
    """
    Implement the Gray-World Color Constancy method on a PyTorch tensor.
    
    Args:
        img: (torch.Tensor) 4D tensor in the format (batch, channels, height, width)
        
    Returns:
        (torch.Tensor): Color corrected image
    """
    assert img.dim() == 4, "Input image tensor should be a 4D tensor"
    assert img.size(1) == 3, "Input image tensor should have 3 channels"
    
    # Calculate the mean color in each channel
    mean_rgb = torch.mean(img, dim=[2, 3], keepdim=True)
    
    # Calculate the average mean across all channels
    mean_g = torch.mean(mean_rgb, dim=1, keepdim=True)
    
    # Scale the channels by their relative illumination
    scale = mean_g / mean_rgb
...
class MyDataset(Dataset):
    def __init__(self, paths, csv_file='metadata.csv', transform=None, inp_size=224, resize = False):
    ...
    def __getitem__(self, idx):
        ...
        img_metadata = self.metadata.loc[self.metadata['isic_id'] == img_id]
        protected_attr = torch.tensor(1.0 if img_metadata['sex'].iloc[0] == 'female' else 0.0, dtype=torch.float32)
        ...
        if self.resize:
            image.thumbnail((self.inp_size, self.inp_size))
...
class MyDataset2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.resize:
            image.thumbnail((self.inp_size, self.inp_size))
        # new_img = img_name.split("/")[2]
        # image.save(f'ISIC18_32/{new_img}')
